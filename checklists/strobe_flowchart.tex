...
\node[mainbox] (A) {%
  ...
  UKHLS all respondents across waves 2-10 (\emph{n}\,=\,96372)
};

\node[mainbox, below=1.6cm of A] (B) {%
  \textbf{Employment-eligible population}\\[2pt]
  Employed or not-employed at risk, aged 25--65, waves 2-10 (\emph{n}\,=\,75177)
};

\node[mainbox, below=1.6cm of B] (C) {%
  \textbf{Pre-baseline observed}\\[2pt]
  Observed at wave~2 (pre-baseline) and in scope (\emph{n}\,=\,40660)
};

\node[mainbox, below=1.6cm of C] (D) {%
  \textbf{Analysed}\\[2pt] 
  Analytical sample (waves 3-6) \\ 
  (\emph{n}\,=\,23767)
};
...
\node[sidebox, anchor=west] (excl1) at ($(mAB) + (4.8cm, 0)$) {%
  ...
  $\bullet$ Not observed at wave~2 (\emph{n}\,=\,18927)
};

\node[sidebox, anchor=west] (excl2) at ($(mBC) + (4.8cm, 0)$) {%
  \textbf{Excluded}\\[2pt]
  $\bullet$ Ever retired / student / long-term sick (\emph{n}\,=\,28883)\\
  $\bullet$ Age outside 25--65 (\emph{n}\,=\,36785)
};

\node[sidebox, anchor=west] (excl3) at ($(mCD) + (4.8cm, 0)$) {%
  \textbf{Insufficient outcome data}\\[2pt]
  $\bullet$ Not observed at outcome wave\\
  \phantom{$\bullet$ }and ${\geq}$2 MCS values missing (\emph{n}\,=\,2876)
};
