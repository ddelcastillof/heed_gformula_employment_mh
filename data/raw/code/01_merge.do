...
* DO-FILE NAME:         01_merge.do
* DESCRIPTION:          Merges individual and household response files
***************************************************************************************
* COUNTRY:              UK
...
***************************************************************************************
/*
* This do-file merges the individual and household response files by:
1. Selecting relevant variables from each file and renaming them to have a consistent wave prefix (w1_, w2_, etc.)
2. Appending the selected variables across waves to create combined datasets for individual and household responses
3. Merging the combined individual and household datasets together using the unique identifiers (pidp, hidp, swv) to create a final merged dataset with all relevant variables for analysis
*/
...
foreach w of global UKHLS_panel_waves {
	...
	rename `w'_* *
	if (`waveno'>1) {
		append using "$dir_out/add_vars_ukhls.dta"
	}
	save "$dir_out/add_vars_ukhls.dta", replace
}
...
foreach w of global UKHLS_panel_waves {
	...
	rename `w'_* *
	if (`waveno'>1) {
		append using "$dir_out/add_vars_ukhls_indresp.dta"
	}
	save "$dir_out/add_vars_ukhls_indresp.dta", replace
}
...
foreach w of global UKHLS_panel_waves {
	...
	rename `w'_* *
	if (`waveno'>1) {
		append using "$dir_out/add_vars_ukhls_hhresp.dta"
	}
	save "$dir_out/add_vars_ukhls_hhresp.dta", replace
} 
...
foreach w of global UKHLS_panel_waves {
	...
	rename `w'_* *
	if (`waveno'>1) {
		append using "$dir_out/add_vars_ukhls_income.dta"
	}
	save "$dir_out/add_vars_ukhls_income.dta", replace
}
...
save "$dir_out/tmp_income", replace
restore
...
save "$dir_out/tmp_ucrcpt", replace
restore
...
foreach w of global UKHLS_panel_waves {
	...
	if (`waveno'>7 | mod(`waveno',2)==0) {
		...
		rename `w'_* *
		if (`waveno'>2) {
			append using "$dir_out/add_vars_ukhls_youth.dta"
		}
		save "$dir_out/add_vars_ukhls_youth.dta", replace
	}
}
...
*************************************************************************************/
use "$dir_out/add_vars_ukhls.dta", clear
merge 1:1 pidp hidp swv using "$dir_out/add_vars_ukhls_indresp.dta", keep(1 3) nogen
merge m:1 hidp swv using "$dir_out/add_vars_ukhls_hhresp.dta", keep(1 3) nogen
merge m:1 hidp swv using "$dir_out/tmp_ucrcpt", keep(1 3) nogen
merge 1:1 pidp hidp swv using "$dir_out/tmp_income", keep(1 3) nogen
merge 1:1 pidp hidp swv using "$dir_out/add_vars_ukhls_youth.dta", keep(1 3) nogen

//Merge variables from cross-wave file (Stable characteristics of individuals)
merge m:1 pidp using "$dir_ukhls_data/xwavedat.dta" , keepusing(maedqf paedqf birthy racel_dv) keep(1 3) nogen 
...
* save output
*************************************************************************************/
save "$dir_out/ukhls_pooled_all_obs_01.dta", replace
cap log close 
